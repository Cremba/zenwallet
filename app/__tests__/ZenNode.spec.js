...
test('zenNodeArgs empty args', () => {
  const zenNode = getZenNode()
  expect(zenNode.zenNodeArgs).toEqual(['--test', '--data-path', 'data/test-beta', '--api', '127.0.0.1:21567'])
})
test('zenNodeArgs wipe from command line', () => {
  ...
  const zenNode = getZenNode()
  // assertion
  expect(zenNode.zenNodeArgs).toEqual(['--wipe', '--test', '--data-path', 'data/test-beta', '--api', '127.0.0.1:21567'])
})
test('zenNodeArgs wipe full from command line', () => {
  ...
  const zenNode = getZenNode()
  // assertion
  expect(zenNode.zenNodeArgs).toEqual(['--wipe', 'full', '--test', '--data-path', 'data/test-beta', '--api', '127.0.0.1:21567'])
})
